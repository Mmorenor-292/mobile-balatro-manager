...
8. To bring the desktop mod folders across separately, connect the phone's writable `Mods` folder once, open **Mods**, and tap the hard-drive import button. MBM requests a bounded LAN ZIP from `%APPDATA%/Balatro/Mods`, rejects desktop-only binaries, quarantines same-name folders instead of deleting them, creates a backup first, and records the operation in **Installation History**. Review the imported mods before enabling them.
