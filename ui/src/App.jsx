...
function ModCard({ mod, catalog, operation, selected, onSelect, onViewHistory }) {
  ...
  const operationActive = Boolean(operation?.active);
  const busy = Boolean(operationActive && ((operation.itemId === catalogItem?.id && operation.source === catalogItem?.source) || operation.itemId === mod.folder));
  const askDelete = () => {
  ...
  return <article className={`mod-card ${active ? "mod-active" : "mod-inactive"}`}>
    ...
    <div className="mod-copy"><h2>{mod.name}</h2><p>{mod.version || "unknown version"} · {mod.folder}</p>{mod.diagnostics?.[0] && mod.severity === "error" && <span className="diagnostic error" title={mod.diagnostics[0]}><CircleAlert />{mod.diagnostics[0]}</span>}{expanded && <div className="mod-details"><p>{mod.description || "No description available."}</p><p><strong>Dependencies:</strong> {(mod.dependencies || []).join(", ") || "None listed"}</p>{catalogItem ? <div className="installed-version-tools"><div className="catalog-version-row"><span>Current: <strong>{mod.version}</strong></span><span>Catalog: <strong>{catalogItem.latestVersion || catalogItem.version}</strong></span></div><label className="catalog-version-picker">Target version<select aria-label={`Version for installed ${mod.name}`} value={selectedVersion} disabled={operationActive} onChange={(event) => setSelectedVersionOverride(event.target.value)}>{releases.map((release) => <option key={release.version} value={release.version}>{release.version}{release.version === mod.version ? " (installed)" : release.version === defaultVersion ? " (latest)" : ""}</option>)}</select></label><button className="version-update-button" type="button" disabled={operationActive || !selectedVersion} onClick={update}>{busy ? <><LoaderCircle className="spin" /> Updating…</> : selectedVersion === mod.version ? "Reinstall selected version" : `Update to ${selectedVersion}`}</button></div> : <p className="catalog-unmatched">No catalog releases matched this local mod yet.</p>}<div className="detail-actions"><button type="button" onClick={onViewHistory}>Installation history</button>{mod.website && <button type="button" onClick={() => invoke("openModWebsite", { url: mod.website })}>Source / license</button>}{imm && <button type="button" disabled={operationActive} onClick={() => invoke("repairImmVersion", { folder: mod.folder })}>Fix IMM mobile version</button>}<button type="button" className="danger-action" disabled={operationActive} onClick={askDelete}>Delete permanently</button></div></div>}</div>
    <div className="mod-actions"><label className="mod-select"><span className="sr-only">Select {mod.name}</span><input type="checkbox" checked={selected} disabled={operationActive} onChange={(event) => onSelect(mod.folder, event.target.checked)} /></label><button className="icon-button mini" type="button" aria-label={`Options for ${mod.name}`} title="Mod options" onClick={() => setExpanded((value) => !value)}><Settings2 /></button><label className="switch"><span className="sr-only">{active ? `Disable ${mod.name}` : `Enable ${mod.name}`}</span><input type="checkbox" checked={active} disabled={operationActive} onChange={(event) => invoke("toggleMod", { folder: mod.folder, hidden: !event.target.checked })} /><span className="switch-track"><span /></span></label></div>
  </article>;
}
...
function CatalogCard({ item, connected, operation }) {
  ...
  const operationActive = Boolean(operation?.active);
  const busy = Boolean(operationActive && operation.itemId === item.id && operation.source === item.source);
  const label = busy ? (operation.kind === "update" ? "Updating…" : "Installing…") : sourceOnly ? "Source only" : item.installed ? (selectedVersion === currentVersion ? "Reinstall" : "Update") : "Install";
  ...
  return <article className="catalog-card">
    ...
    <label className="catalog-version-picker">Version<select aria-label={`Version for ${item.name}`} value={selectedVersion} disabled={operationActive} onChange={(event) => setSelectedVersionOverride(event.target.value)}>{releases.map((release) => <option key={release.version} value={release.version}>{release.version}{release.version === currentVersion ? " (installed)" : release.version === latestVersion ? " (latest)" : ""}</option>)}</select></label>
    <div className="catalog-actions"><button className={item.installed ? "installed-button" : "install-button"} type="button" disabled={!installable || !connected || operationActive} title={sourceOnly ? "This entry has no downloadable archive yet." : !connected ? "Connect your Mods folder before installing." : undefined} onClick={runInstall}>{busy && <LoaderCircle className="spin" />}{label}</button>{item.homepage && <button className="icon-button mini" type="button" aria-label={`Open source for ${item.name}`} onClick={() => invoke("openModWebsite", { url: item.homepage })}><ChevronRight /></button>}</div>
  </article>;
}
