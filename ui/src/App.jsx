...
function ModsScreen({ state, query, setQuery, filter, setFilter, onViewHistory }) {
  ...
  const bulkDelete = () => { if (!selectedFolders.length) return; if (window.confirm(`Move ${selectedFolders.length} selected mod(s) to reversible quarantine?`)) invoke("deleteMods", { folders: selectedFolders }); };
  return <section className="screen screen-mods"><div className="screen-heading"><div><p className="eyebrow">LIBRARY</p><h2>Mods</h2></div><div className="screen-actions"><button className="icon-button bordered" type="button" aria-label="Import mod ZIP" onClick={() => invoke("importMod")}><Upload /><span className="sr-only">Import mod ZIP</span></button><button className="icon-button bordered" type="button" aria-label="Import mod folder" onClick={() => invoke("importModFolder")}><FolderOpen /><span className="sr-only">Import mod folder</span></button>{state.desktopPaired && state.desktopModSummary?.available && <button className="icon-button bordered" type="button" aria-label="Import desktop mods" title="Import desktop mods" onClick={() => invoke("importDesktopMods")}><HardDrive /><span className="sr-only">Import desktop mods</span></button>}</div></div><div className="health-card compact-health"><div className="stat-grid"><Stat value={state.counts?.active || 0} label="active" tone="success" /><Stat value={state.counts?.hidden || 0} label="inactive" /><Stat value={visibleMods.length} label="installed" tone="info" /></div><div className="health-actions"><button type="button" aria-label="Save backup" title="Save backup" onClick={() => invoke("saveSnapshot")}><Save /></button><button type="button" aria-label="Refresh mods" title="Refresh mods" onClick={() => invoke("refresh")}><RefreshCw /></button><button type="button" aria-label="Undo last change" title="Undo last change" onClick={() => invoke("undo")} disabled={!state.canUndo}><Undo2 /></button></div></div><div className="search-row"><label className="search-box"><Search /><span className="sr-only">Search mods</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="SEARCH MODS" /></label><button className="icon-button bordered" type="button" aria-label="Open filter options" onClick={() => setFilter(filter === "all" ? "active" : filter === "active" ? "inactive" : "all")}><Settings2 /></button></div><div className="filter-strip mod-filter-strip"><div className="select-control"><Filter aria-hidden="true" /><select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter mods by category"><option>All</option><option>Technical</option><option>Content</option><option>Visual</option><option>Quality of Life</option></select></div><div className="select-control"><ArrowDownAZ aria-hidden="true" /><select value={sort} onChange={(event) => setSort(event.target.value)} aria-label="Sort mods"><option value="name">Name</option><option value="updated">Last updated</option><option value="status">Active first</option></select></div></div><div className="segments" role="tablist" aria-label="Filter mods">{filters.map((value) => <button key={value} type="button" role="tab" aria-selected={filter === value} className={filter === value ? "selected" : ""} onClick={() => setFilter(value)}>{value}</button>)}</div>{selectedFolders.length > 0 && <div className="selection-actions"><span>{selectedFolders.length} selected</span><button type="button" onClick={() => bulkToggle(false)}>Enable</button><button type="button" onClick={() => bulkToggle(true)}>Disable</button><button type="button" onClick={bulkDelete}>Delete selected</button><button type="button" onClick={clearSelection}>Clear</button></div>}<div className="list-heading"><p>{visibleMods.length} mods</p></div><div className="mod-list">{visibleMods.map((mod) => <ModCard key={mod.folder} mod={mod} selected={selected.has(mod.folder)} onSelect={updateSelection} onViewHistory={onViewHistory} />)}{visibleMods.length === 0 && <div className="empty-state"><Search /><h2>No mods found</h2><p>Try another filter or search term.</p></div>}</div></section>;
}
function Stat({ value, label, tone = "muted" }) { return <div className={`stat ${tone}`}><strong>{value}</strong><span>{label}</span></div>; }
function ModCard({ mod, selected, onSelect, onViewHistory }) { const [expanded, setExpanded] = useState(false); const active = !mod.hidden; const image = mod.thumbnail || mod.icon || ""; const framework = /steamodded|lovely|framework/i.test(`${mod.name} ${mod.folder}`); const askDelete = () => { const dependencyText = (mod.dependents || []).length ? `\nDependents: ${mod.dependents.join(", ")}.` : ""; const frameworkText = framework ? "\nThis looks like a framework; dependent mods may stop working." : ""; const confirmed = window.confirm(`Move ${mod.name} to reversible quarantine? It will not be permanently deleted.${dependencyText}${frameworkText}`); if (confirmed) invoke("deleteMod", { folder: mod.folder }); }; return <article className={`mod-card ${active ? "mod-active" : "mod-inactive"}`}><div className="mod-icon">{image ? <img src={image} alt={`${mod.name} icon`} /> : <Puzzle />}</div><div className="mod-copy"><h2>{mod.name}</h2><p>{mod.version || "unknown version"} · {mod.folder}</p>{mod.diagnostics?.[0] && mod.severity === "error" && <span className="diagnostic error" title={mod.diagnostics[0]}><CircleAlert />{mod.diagnostics[0]}</span>}{expanded && <div className="mod-details"><p>{mod.description || "No description available."}</p><p><strong>Dependencies:</strong> {(mod.dependencies || []).join(", ") || "None listed"}</p><div className="detail-actions"><button type="button" onClick={onViewHistory}>Installation history</button>{mod.website && <button type="button" onClick={() => invoke("openModWebsite", { url: mod.website })}>Source / license</button>}<button type="button" onClick={() => invoke("chooseCatalogVersion", { id: mod.folder })}>Choose version</button><button type="button" onClick={askDelete}>Delete mod</button><button type="button" onClick={() => invoke("restoreInstall", { id: mod.folder })}>Restore last version</button></div></div>}</div><div className="mod-actions"><label className="mod-select"><span className="sr-only">Select {mod.name}</span><input type="checkbox" checked={selected} onChange={(event) => onSelect(mod.folder, event.target.checked)} /></label><button className="icon-button mini" type="button" aria-label={`Options for ${mod.name}`} title="Mod options" onClick={() => setExpanded((value) => !value)}><Settings2 /></button><label className="switch"><span className="sr-only">{active ? `Disable ${mod.name}` : `Enable ${mod.name}`}</span><input type="checkbox" checked={active} onChange={(event) => invoke("toggleMod", { folder: mod.folder, hidden: !event.target.checked })} /><span className="switch-track"><span /></span></label></div></article>; }
...
function DiscoverScreen({ state }) {
  ...
  return <section className="screen discover-screen"><div className="screen-heading"><div><p className="eyebrow">CURATED SOURCES</p><h2>Discover</h2></div><button className="icon-button bordered" type="button" aria-label="Refresh catalog" onClick={() => invoke("loadCatalog")}><RefreshCw /></button></div><p className="lead">Browse and install new mods</p><div className="search-row"><label className="search-box"><Search /><span className="sr-only">Search catalog</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="SEARCH THE CATALOG" /></label><button className="icon-button bordered" type="button" aria-label="Filter and sort" onClick={() => setSort(sort === "popular" ? "updated" : sort === "updated" ? "name" : "popular")}><Settings2 /></button></div><div className="filter-strip"><div className="select-control"><Filter aria-hidden="true" /><select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter by category">{categories.map((item) => <option key={item}>{item}</option>)}</select></div><div className="select-control"><ArrowDownAZ aria-hidden="true" /><select value={sort} onChange={(event) => setSort(event.target.value)} aria-label="Sort catalog"><option value="popular">Popular</option><option value="updated">Last updated</option><option value="name">Name</option></select></div></div><div className="source-tabs" role="tablist" aria-label="Catalog source">{sources.map((value) => <button key={value} type="button" role="tab" aria-selected={source === value} className={source === value ? "selected" : ""} onClick={() => setSource(value)}>{value === "All" ? "All" : value === "BMI" ? "Mod Index" : value}</button>)}</div><div className="catalog-list">{results.map((item) => <CatalogCard key={`${item.source}:${item.id}`} item={item} connected={state.connected} />)}{results.length === 0 && <div className="empty-state"><PackageSearch /><h2>No catalog matches</h2><p>Try another category or source.</p></div>}</div></section>;
}
function CatalogCard({ item, connected }) {
  ...
  return <article className="catalog-card">
    ...
    <div className="catalog-actions"><button className={item.installed ? "installed-button" : "install-button"} type="button" disabled={!installable} title={sourceOnly ? "This entry has no downloadable archive yet." : !connected ? "Connect your Mods folder before installing." : undefined} onClick={runInstall}>{label}</button>{item.homepage && <button className="icon-button mini" type="button" aria-label={`Open source for ${item.name}`} onClick={() => invoke("openModWebsite", { url: item.homepage })}><ChevronRight /></button>}</div>
  </article>;
}
...
function HistoryScreen({ state, tab, setTab }) {
  ...
  return <section className="screen history-screen">
    ...
    <p className="lead">Local records only. Restore, export, reinstall, rollback, or remove records without touching the other tab.</p>
    <div className="history-list">{entries.map((item) => {
      const installEntry = tab === "installs";
      const canRestore = installEntry && ["quarantine", "update"].includes(item.kind);
      const canReinstall = installEntry && ["install", "update"].includes(item.kind);
      const canRollback = installEntry && item.kind === "update";
      ...
      return <article key={item.id}><div className="history-icon">{tab === "backups" ? <ArchiveRestore /> : <Download />}</div><div><h3>{title}</h3><p>{details}</p></div><div className="history-actions">
        ...
        {canRollback && <button type="button" onClick={() => invoke("restoreInstall", { id: item.id })}>Rollback</button>}
        {!canRollback && canRestore && <button type="button" onClick={() => invoke("restoreInstall", { id: item.id })}>Restore</button>}
...
function Toast({ message }) { return <div className="toast" role="status">{message}</div>; }
