...
function CatalogCard({ item, connected, operations, globalBusy }) {
  ...
  const busy = Boolean(operation);
  const blocked = globalBusy || busy;
  const label = busy ? operationText(operation) : sourceOnly ? "Source only" : item.installed ? (selectedVersion === currentVersion ? "Reinstall" : "Update") : "Install";
  ...
  return <article className="catalog-card">
    ...
    <div className="catalog-version-row"><span>Installed: <strong>{currentVersion}</strong></span><span>Latest: <strong>{item.versionKind === "source-revision" ? `source · ${latestVersion}` : latestVersion}</strong></span></div>
